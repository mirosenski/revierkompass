  ...
  const setupMapSources = () => {
    ...
    routeResults.forEach((route) => {
      ...
      try {
        ...
        // Prüfe ob es sich um eine Fallback-Route handelt
        const isFallback = route.route?.isFallback || false;
        
        // Add route line layer (mit unterschiedlichem Stil für Fallback-Routen)
        map.current!.addLayer({
          ...
          paint: {
            'line-color': isFallback ? '#ff9800' : route.color, // Orange für Fallback-Routen
            'line-width': [
              'case',
            ...
            'line-opacity': isFallback ? 0.6 : 0.8, // Transparenter für Fallback
            'line-dasharray': isFallback ? [3, 3] : [1, 0] // Gestrichelte Linie für Fallback
          }
        });
