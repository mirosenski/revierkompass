...
export function VirtualizedHierarchy({
  ...
  const [revierOrder, setRevierOrder] = useState<string[]>([]);
  ...
  const allReviere = useMemo(() => {
    ...
    praesidien.forEach(praesidium => {
      if (expandedPraesidien.has(praesidium.id)) {
        ...
        const praesidiumReviere = availableReviere.filter(revier => 
          revier.praesidiumId === praesidium.id
        );
